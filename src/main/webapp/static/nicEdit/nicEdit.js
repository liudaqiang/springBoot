 ...
 */
/*
 * Modified by Hervey Hall <mail@herveyhall.cf> at 2017/6/11.
 */
...
var nicUploadButton = nicEditorAdvancedButton.extend({
	nicURI : "/picture/upload",
	errorText : "上传文件失败！",
	addPane : function() {
		if (typeof window.FormData === "undefined") {
			return this.onError("非常抱歉，您的浏览器不支持文件上传！")
		}
		this.im = this.ne.selectedInstance.selElm().parentTag("IMG");
		var A = new bkElement("div").setStyle({
			padding : "10px"
	...
	onUploaded : function(B) {
		...
		if (!this.im) {
			...
			this.ne.nicCommand("insertImage", D["src"]);
			this.im = this.findElm("IMG", "src", D["src"])
		}
...
nicEditor = nicEditor.extend({
	floatingPanel : function() {
		...
		this.addEvent("focus", this.reposition.closure(this)).addEvent("blur",
				this.hide.closure(this));
