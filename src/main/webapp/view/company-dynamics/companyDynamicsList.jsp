...
<body>
	...
	<div class="main-container" id="main-container">
		...
		<div class="main-container-inner">
				...
				<div class="page-content">
					<div class="page-header">
						<h1 style="display: inline-block;">
							动态管理 <small> <i class="icon-double-angle-right"></i> 动态列表
							</small>
						</h1>
						<button class="btn btn-primary btn-sm"
							style="float: right; margin-right: 30px;"
							onclick="javascript:window.location.href='${ctx}/admin/companyDynamics/edit'">创建动态</button>
					</div>
					<!-- /.page-header -->
					<div class="row">
						<div class="col-xs-12">
								...
								<div class="col-xs-12">
									<div class="table-responsive">
										<table id="sample-table-1"
											...
											<tbody>
												<c:forEach items="${companyDynamicsList}"
													var="companyDynamics">
													<tr>
														<td>${companyDynamics.id}</td>
														<td>${companyDynamics.title}</td>
														<td class="hidden-480">${companyDynamics.subTitle}</td>
														<!-- 
															<c:set var="aaa" value="${companyDynamics.id}"></c:set>
															<td>${companyMap.aaa.name}</td>
															-->
														<td>${companyDynamics.companyId}</td>
														...
														<td>
															<div
																...
																<button class="btn btn-xs btn-info"
																	onclick="javascript:window.location.href='${ctx}/admin/companyDynamics/edit?id=${companyDynamics.id}'">
																	<i class="icon-edit bigger-120"></i>
																</button>

																<button class="btn btn-xs btn-danger"
																	onclick="deletePerson('${companyDynamics.id}')">
							...
							<div class="center">
								<ul class="pagination">
									...
									<c:forEach begin="1" end="${pageCount}" var="page">
										<li class="active"><a
											href="${ctx}/admin/companyDynamics/list?father=400&children=4001&page=${page}">${page}</a>
										</li>
									</c:forEach>
			...
			<script type="text/javascript">
				function deletePerson(companyDynamicsId) {
					$
							.confirm({
								title : '是否确定删除?',
								content : '删除后数据将无法恢复',
									...
									confirm : {
										...
										action : function() {
											window.location.href = '${ctx}'
													+ "/admin/companyDynamics/delete?id="
													+ companyDynamicsId;
										}
									},
									cancelAction : {
