...
<body>
	...
	<div class="main-container" id="main-container">
		...
		<div class="main-container-inner">
				...
				<div class="page-content">
					<div class="page-header">
						<h1 style="display: inline-block;">
							项目风采管理 <small> <i class="icon-double-angle-right"></i> 风采列表
							</small>
						</h1>
						<button class="btn btn-primary btn-sm"
							style="float: right; margin-right: 30px;"
							onclick="javascript:window.location.href='${ctx}/admin/projectMien/edit'">创建动态</button>
					</div>
					<!-- /.page-header -->
					<div class="row">
						<div class="col-xs-12">
								...
								<div class="col-xs-12">
									<div class="table-responsive">
										<table id="sample-table-1"
											...
											<tbody>
												<c:forEach items="${projectMienList}"
													var="projectMien">
													<tr>
														<td>${projectMien.id}</td>
														<td>${projectMien.name}</td>
														<td class="hidden-480">${projectMien.projectMienType}</td>
														<!-- 
															<c:set var="aaa" value="${projectMien.id}"></c:set>
															<td>${companyMap.aaa.name}</td>
															-->
														<td>${projectMien.projectId}</td>
														...
														<td>
															<div
																...
																<button class="btn btn-xs btn-info"
																	onclick="javascript:window.location.href='${ctx}/admin/projectMien/edit?id=${projectMien.id}'">
																	<i class="icon-edit bigger-120"></i>
																</button>

																<button class="btn btn-xs btn-danger"
																	onclick="deletePerson('${projectMien.id}')">
							...
							<div class="center">
								<ul class="pagination">
									...
									<c:forEach begin="1" end="${pageCount}" var="page">
										<li class="active"><a
											href="${ctx}/admin/projectMien/list?father=400&children=4001&page=${page}">${page}</a>
										</li>
									</c:forEach>
			...
			<script type="text/javascript">
				function deletePerson(projectMienId) {
					$
							.confirm({
								title : '是否确定删除?',
								content : '删除后数据将无法恢复',
