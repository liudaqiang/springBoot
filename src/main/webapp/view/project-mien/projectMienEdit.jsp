			...
			<div class="main-content">
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li><a href="#">项目风采管理</a></li>
						<li class="active">风采修改</li>
					</ul>
					<!-- .breadcrumb -->
				</div>
				...
				<div class="page-content">
					<div class="page-header">
						<h1>
							项目风采管理 <small> <i class="icon-double-angle-right"></i>
								风采修改
							</small>
						</h1>
					</div>
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<form class="form-horizontal" role="form"
								<c:if test="${projectMien != null}">action="${ctx}/admin/projectMien/update"</c:if>
								<c:if test="${projectMien == null}">action="${ctx}/admin/projectMien/add"</c:if>
								method="POST">
								<input type="hidden" value="${projectMien.id}" name="id">
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input name="name" type="text" class="col-xs-10 col-sm-5"
											id="form-input-readonly" value="${projectMien.name}" />
									</div>
								</div>
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"
										...
										name="projectMienType">
										<option value="1"
											<c:if test="${'1' == projectMien.projectMienType}">selected="selected"</c:if>>党员示范岗</option>
										<option value="2"
											<c:if test="${'2' == projectMien.projectMienType}">selected="selected"</c:if>>青年突击</option>
										<option value="3"
											<c:if test="${'3' == projectMien.projectMienType}">selected="selected"</c:if>>队立功竞赛</option>
										<option value="4"
											<c:if test="${'4' == projectMien.projectMienType}">selected="selected"</c:if>>每月之星</option>
									</select>
								</div>
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"
										for="form-field-2"> 照片</label>

									<div class="col-sm-9">
										<img id="picture-img" alt="照片 " src="${projectMien.picture}"
											style="width: 300px;"> <input type="file"
											id="picture-upload"> <input type="hidden"
											id="picture" name="picture" value="${projectMien.picture}">
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div id="sample" class="col-sm-9">
										...
										<textarea name="description"
											style="width: 500px; height: 100px; overflow: scroll;">${projectMien.description}</textarea>
									</div>
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"
										for="form-field-2"> 所属项目</label>

									<div class="col-sm-9">
										<select class="col-xs-10 col-sm-5 chosen-select"
											id="form-field-select-3" data-placeholder="选择项目"
											name="projectId">
											<c:forEach items="${projectList}" var="project">
												<option value="${project.id}"
													<c:if test="${project.id == projectMien.projectId}">selected="selected"</c:if>>${project.name}</option>
											</c:forEach>
										</select>
									</div>
								</div>
