...
public class SuffixAdminController {
	@Autowired
	...
	public String add(Dict dict, Model model, HttpServletRequest request) {
		//字典类型  专业
		dict.setDictType(FinalType.DICT_SUFFIX);
		if (StringUtils.isNotBlank(dict.getName())) {
			...
			return "redirect:/admin/suffix/list";
		}
	...
	public String findList(Model model,@RequestParam(value = "page", defaultValue = "1") Integer pageNum) {
		...
		List<Dict> suffixList = dictPage.getRows();
		//********************************************************
		/*TODO*/
	...
	public String edit(String id, Model model) {
		if (StringUtils.isNotBlank(id)) {
			...
			model.addAttribute("suffix", dict);
		}
		return "/view/suffix/suffixEdit";
	}
